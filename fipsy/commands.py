...
def scan(pin: bool) -> None:
    ...
    for peer_id, ipns_keys in results:
        click.echo(f"Peer Index: http://ipfs.io/ipns/{peer_id}")
        ...
        for name, (ipns_name, resolved) in ipns_keys.items():
            if resolved:
                ...
                click.echo(f"  {name} (IPNS): http://ipfs.io/ipns/{ipns_name}")
                click.echo(f"  {name} (IPFS): http://ipfs.io/ipfs/{cid}")
                if pin:
                    if _pin_cid(cid):
                ...
                click.echo(f"  {name}: unresolved... (http://ipfs.io/ipns/{ipns_name})")
                db.upsert_discovered(peer_id, ipns_name, name=name)
        click.echo()
...
def _publish_entry(key: str, dir_path: Path, ipns_name: str) -> str | None:
    ...
    try:
        ...
        click.echo(f"  {key}: http://ipfs.io/ipns/{ipns_name}")
        click.echo(f"  {key}: http://ipfs.io/ipfs/{cid}")
        return cid
    except subprocess.CalledProcessError:
...
def index() -> None:
    ...
    if keys:
        ...
        for key, ipns_name in keys.items():
            ...
            click.echo(
                f"  {display_name}: http://ipfs.io/ipns/{ipns_name}{path_suffix}"
            )
    else:
...
def add(dir_path: Path) -> None:
    ...
    click.echo(f"http://ipfs.io/ipfs/{cid}")
    ...
    click.echo(f"http://ipfs.io/ipns/{ipns_name}")
...
def publish() -> None:
    ...
    try:
        ...
        click.echo(f"  http://ipfs.io/ipns/{ipfs.node_id()}")
        click.echo(f"  http://ipfs.io/ipfs/{cid}")
    finally:
...
def _write_index_html(directory: Path, keys: dict[str, str]) -> None:
    ...
    for key_name, ipns_name in keys.items():
        lines.append(
            f'    <li><a href="http://ipfs.io/ipns/{ipns_name}">{key_name}</a> <code>{ipns_name}</code></li>'
        )
    lines.extend(
        [
            "  </ul>",
